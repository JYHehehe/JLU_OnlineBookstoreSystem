...
public class Order_service {
    ...
    public List<Order> query_refund_order_list(int user_id){

        List<Order> order_list=order_mapper.query_refund_order_list(user_id);
        for(int i=0;i<order_list.size();i++){
            int goods_id=order_list.get(i).getGoods_id();
            order_list.get(i).setGoods_name(goods_mapper.query_goods_name(goods_id));
            order_list.get(i).setView_address(goods_mapper.query_goods_view_address(goods_id));
    ...
    public int req_refund(int order_id){
        return order_mapper.set_order_state_req_refund(order_id);
    }
